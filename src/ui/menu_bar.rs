...
pub fn create_menubar(app: &Application) {
    ...
    edit_menu.append(Some("Enable Playlistview"), Some("app.menu_edit_playlistview_switch"));
    edit_menu.append(Some("Export Playlistview"), None);
    edit_menu.append(Some("Import Playlistview"), None);

    let view_menu = Menu::new();
    view_menu.append(Some("Show Gif in Config Window"), Some("app.menu_view_gif_switch"));
    view_menu.append(Some("Enable DarkMode"), Some("app.menu_view_darkmode_switch"));

    let songwindow_menu = Menu::new();
    songwindow_menu.append(Some("Refresh"), None);
    songwindow_menu.append(Some("Show Thumbnails"), Some("app.menu_songwindow_thumbnail_switch"));
    songwindow_menu.append(Some("Show Next Dance"), Some("app.menu_songwindow_nextdance_switch"));
...
pub fn register_menu_actions(app: &Application) {
    let file_open_m3u = ActionEntry::builder("menu_file_open_m3u").activate(|app: &Application, _, _| handle_file_open_m3u(app)).build();
    let exit = ActionEntry::builder("menu_file_exit").activate(|app: &Application, _, _| app.quit()).build();

    app.add_action_entries([file_open_m3u, exit]);
}

pub fn register_actions(app: &Application) {
    app.add_action(&create_switch_action("menu_edit_playlistview_switch", handle_edit_playlistview_switch_action));
    app.add_action(&create_switch_action("menu_view_gif_switch", handle_view_gif_switch_action));
    app.add_action(&create_switch_action("menu_view_darkmode_switch", handle_view_darkmode_switch_action));
    app.add_action(&create_switch_action("menu_songwindow_thumbnail_switch", handle_songwindow_thumbnail_switch_action));
    app.add_action(&create_switch_action("menu_songwindow_nextdance_switch", handle_songwindow_nextdance_switch_action));
}
...
fn create_switch_action(name: &str, handler: fn(bool)) -> SimpleAction {
    ...
    switch_action.connect_activate(clone!(@strong switch => move |action, _| {
...
fn handle_file_open_m3u(app: &Application) {
    ...
    FileDialog::builder().title("Select m3u file").accept_label("Load").filters(&m3u_select_filters).build()
        .open(Some(&app.windows()[0]), gio::Cancellable::NONE, move |file| {
            if let Ok(file) = file {
